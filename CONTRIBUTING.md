...
`actions.js`, `folders.js`, `archive.js`, `settings.js`, `upload.js`); a new
one has to be added to `scripts` in `internal/web/static_test.go` too, which
is what keeps the page and the tests honest about each other.
